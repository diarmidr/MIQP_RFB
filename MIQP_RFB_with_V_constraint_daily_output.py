...
for i in years:
	...
	# The below code generates a cell voltage list to be constrained, or used in the objective.
	...
	#First, the cell voltage is calculated as the sum of the OCV and the over-potential
        VCelltracker = []
        for i in model.I:
    	    VCell = OCV_data[i] + model.Vfaradaic + model.ASR * (model.CurrentDensityCharge[i] - model.CurrentDensityDischarge[i])
